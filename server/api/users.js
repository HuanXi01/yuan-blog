...
exports.register = (req, res) => {
    ...
    db.query(checkSql, [username], (err, results) => {
        ...
        db.query(insertSql, [username, md5(password), nickname || '', 'user', 0], (err, results) => {
            if (err) {
...
exports.updateUser = (req, res) => {
    ...
    db.query(checkSql, [id], (err, results) => {
        ...
        if (username) {
            ...
            db.query(checkUsernameSql, [username, id], (checkErr, checkResults) => {
                if (checkErr) {
                ...
                if (password && password.trim() !== '') {
                    ...
                    db.query(updateSql, [username, nickname, md5(password), id], (err, results) => {
                        if (err) {
                    ...
                    db.query(updateSql, [username, nickname, id], (err, results) => {
                        if (err) {
                            logger.error('更新用户失败: 数据库更新错误', { error: err.message, id: id })
                            return res.json({ status: 500, message: '更新失败' })
                        }
            ...
            if (password && password.trim() !== '') {
                ...
                db.query(updateSql, [nickname, md5(password), id], (err, results) => {
                    if (err) {
                        logger.error('更新用户失败: 数据库更新错误', { error: err.message, id: id })
                        return res.json({ status: 500, message: '更新失败' })
                    }
                ...
                db.query(updateSql, [nickname, id], (err, results) => {
                    if (err) {
                        logger.error('更新用户失败: 数据库更新错误', { error: err.message, id: id })
                        return res.json({ status: 500, message: '更新失败' })
                    }
...
exports.deleteUser = (req, res) => {
    ...
    db.query(checkSql, [id], (err, results) => {
        ...
        if (results[0].role === 'admin') {
            logger.warn('删除用户失败: 管理员不可删除', { id: id, username: results[0].username })
